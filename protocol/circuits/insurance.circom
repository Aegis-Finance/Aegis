...
/**
 * @title Insurance Circuit
 * @dev ZK circuit for private insurance operations
 * @notice Proves insurance coverage and claims without revealing policyholder identity
 */
template Insurance() {
    // Public inputs
    ...
    commitmentHasher.inputs[1] <== coverageAmount;
    commitmentHasher.inputs[2] <== policyId;
    ...
    // Amount range check
    rangeCheck.in[0] <== coverageAmount;
    rangeCheck.in[1] <== 0;
    rangeCheck.out === 1;
...
component main {public [nullifierHash, commitmentHash, merkleRoot, coverageAmount, policyId]} = Insurance();
