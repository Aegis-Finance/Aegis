...
/**
 * @title DerivativeVerifierWrapper
 * @dev Factory-based wrapper for derivative ZK proof verification
 ...
 */
contract DerivativeVerifierWrapper is IFactoryVerifierWrapper {
    /// @notice Immutable factory reference for gas optimization
    /// @dev Immutable factory reference for gas optimization
    VerifierFactory public immutable VERIFIER_FACTORY;
    
    /// @notice Circuit type constant for this wrapper
    /// @dev Circuit type constant for this wrapper
    string public constant CIRCUIT_TYPE = "derivative";
    
    /// @notice Function selector for verifyProof function
    /// @dev Precomputed selector for gas optimization
     ...
     */
    function getVerifierAddress() public view override returns (address verifierAddress) {
        return VERIFIER_FACTORY.getVerifier(CIRCUIT_TYPE);
    }
    
    /**
     * @notice Verify a ZK proof using the factory-managed verifier
    ...
    /**
     * @notice Check if verifier is available and valid
