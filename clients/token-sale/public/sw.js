/**
 * Service Worker for DDoS Protection (Token Distribution Frontend)
 * Same protection as main frontend
 */

const CACHE_VERSION = 'aegis-token-dist-v1'
const CACHE_NAME = `aegis-token-dist-cache-${CACHE_VERSION}`
...
async function processQueue(event) {
  ...
  while (requestQueue.length > 0 && !isRateLimited()) {
    ...
    // Small delay between requests (~15 requests/second max)
