...
async function fetchBridgeContext(): Promise<{
  ...
  if (!res.ok) {
    return { layer: undefined, isTestnet: ENV.chainId === 14601 }
  }
  const j = (await res.json()) as {
    sonicMainnet?: ChainPackSlice
    sonicTestnet?: ChainPackSlice
  }
  const isTestnet = ENV.chainId === 14601
...
export default function SonicGatewayBridgePanel() {
  ...
  const q = useQuery({
    queryKey: ['td-sonic-gateway-bridge', ENV.chainId],
  ...
  const tokensQ = useQuery({
    queryKey: ['td-bridge-token-options', packRows.map((r) => r.tokenSymbol).join(',')],
  ...
  const balanceQ = useQuery({
    queryKey: ['td-eth-balance', address, selected?.ethereumAddress],
  ...
  const allowanceQ = useQuery({
    queryKey: ['td-eth-allowance', address, selected?.ethereumAddress],
    queryFn: () =>
      fetchEthAllowance(
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: ['td-eth-allowance'] })
    } catch (e) {
