...
export class BrowserFingerprint {
  ...
  /**
   * Get fingerprint ID
   * @remarks Default: session-only id (no canvas/WebGL/audio). Set `VITE_ENABLE_CLIENT_FINGERPRINT=1` for legacy DDoS id.
   * See `docs/PRIVACY_DEFAULTS_AND_FINGERPRINTING.md`.
   */
  static getId(): string {
  ...
  enqueue(
    ...
    if (duplicate) {
      ...
      if (this.getPriorityValue(priority) > this.getPriorityValue(duplicate.priority)) {
        ...
        // We can't easily cancel the old promise, but we can replace the execution logic
        // For simplicity, we'll just let the old one resolve with the new result if possible
        // or just accept that the old one was replaced in queue.
        // Actually, replacing in queue is tricky because the old promise expects a resolution.
        
        // Better strategy: update priority of existing request
        duplicate.priority = priority
        duplicate.timestamp = Date.now() // bump to back of priority group? or keep spot?
        // If we want to upgrade priority, we just update the field.
        // And we return the EXISTING promise.
...
export class CircuitBreaker {
  ...
  private readonly halfOpenMaxAttempts = 3 // Max attempts in half-open state
...
export class DDoSProtectionManager {
  ...
  static async protectedRequest<T>(
    ...
    // RPC eth_call traffic is already gated in SecureRpcProvider (`checkRateLimit('rpc')`).
    // Counting every eth_call against the general API bucket breaks sale bootstrap (~13+ reads on mount).
