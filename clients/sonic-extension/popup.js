...
const ARWEAVE_GATEWAYS = ["https://arweave.net/", "https://arweave.live/", "https://arweave.dev/"]
...
function isAllowedUserNavigationUrl(url) {
  try {
    ...
    if (h === "arweave.net" || h === "arweave.live" || h === "arweave.dev") return true
    if (h === "gateway.irys.xyz" || h.endsWith(".irys.xyz")) return true
    return false
  } catch (e) {
    return false
  }
...
function setResolvedUrl(url) {
  var el = document.getElementById("resolvedUrl")
  if (!el) return
  el.textContent = url || ""
}
...
function openResolvedDomain(domain) {
  setStatus("Resolving on Sonic…", "")
  return resolveDomainToUrl(domain).then(function (url) {
    setResolvedUrl(url)
    if (!chrome || !chrome.tabs) {
      setStatus("Browser tabs API is not available.", "error")
      return
    }
    if (!isAllowedUserNavigationUrl(url)) {
      setStatus("Blocked: URL host is not allowlisted.", "error")
      return
    }
    ...
    if (domain && domain.trim().toLowerCase() === DEFAULT_DOMAIN) {
      if (!chrome || !chrome.tabs) {
        setStatus("Browser tabs API is not available.", "error")
        return
      }
      ...
      if (!isAllowedUserNavigationUrl(fallbackUrl)) {
        setStatus("Invalid fallback URL.", "error")
        return
      }
      chrome.tabs.create({ url: fallbackUrl }, function () {
        if (chrome.runtime.lastError) {
          setStatus("Failed to open tab.", "error")
        } else {
          window.close()
    ...
    setStatus(err && err.message ? err.message : "Failed to resolve domain.", "error")
  })
}
...
function handleOpenClick() {
  ...
  if (!input) {
    openResolvedDomain(DEFAULT_DOMAIN)
    return
  }
...
function init() {
  ...
  setStatus("Ready to resolve .sonic domains via Sonic.", "ok")
}
